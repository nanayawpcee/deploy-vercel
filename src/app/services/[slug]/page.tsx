...
export default function ServiceDetailPage({ params }: Props) {
  ...
  return (
    <>
      ...
      <section style={{ padding: "5rem 2rem", background: "#fff" }}>
        <div className="container" style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: "4rem", alignItems: "start" }}>
          ...
          <AnimateIn>
            ...
            <p style={{ color: "var(--text-light)", lineHeight: 1.82, fontSize: "1rem", marginBottom: "2.5rem" }}>
              {svc.fullDesc}
            </p>

            <h3 style={{ fontFamily: "var(--font-serif)", fontSize: "1.15rem", fontWeight: 700, color: "var(--text-dark)", margin: "0 0 1rem" }}>
              What We Offer
            </h3>
            <ul style={{ listStyle: "none", padding: 0, margin: "0 0 2.5rem", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.65rem" }}>
              {svc.features.map(feat => (
                <li key={feat} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div style={{ width: 22, height: 22, borderRadius: "50%", background: "var(--light-green)", border: "1.5px solid var(--primary)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, flexShrink: 0 }}>✓</div>
                  <span style={{ fontSize: "0.88rem", color: "var(--text-mid)", fontWeight: 500 }}>{feat}</span>
                </li>
              ))}
            </ul>
          ...
          <aside className="desktop-only-sidebar" style={{ position: "sticky", top: 90 }}>
            <AnimateIn delay={150} direction="right">
              <div style={{ background: "var(--primary)", borderRadius: "var(--radius-lg)", padding: "1.75rem", marginBottom: "1.5rem" }}>
                ...
                <p style={{ color: "rgba(255,255,255,0.7)", fontSize: "0.84rem", lineHeight: 1.65, margin: "0 0 1.25rem" }}>
                  Schedule a consultation for {svc.title}. Our team will confirm your slot within 24 hours.
                </p>
                <BookButton dept={svc.title} style={{ width: "100%", justifyContent: "center", background: "var(--accent)", color: "var(--text-dark)" }} />
              </div>
              ...
              <div style={{ background: "var(--off-white)", borderRadius: "var(--radius-md)", padding: "1.5rem", border: "1.5px solid #E2EBE7" }}>
                ...
                {[
                  ...
                  <div key={dept} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid #E2EBE7", fontSize: "0.83rem" }}>
                    <span style={{ color: "var(--text-mid)", fontWeight: 600 }}>{dept}</span>
      ...
      {/* Other services */}
      <section style={{ padding: "4rem 2rem 5rem", background: "var(--off-white)" }}>
        <div className="container">
          <h3 style={{ fontFamily: "var(--font-serif)", fontSize: "1.3rem", fontWeight: 800, color: "var(--text-dark)", marginBottom: "1.75rem" }}>
            Other Services
          </h3>
